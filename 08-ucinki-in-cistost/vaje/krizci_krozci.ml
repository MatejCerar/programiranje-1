...
(*TI ŠE NE DELA PREPIŠI IZ REŠITEV*)
...
let rec winner_of_list triples : player option =
  match triples with 
  | [] -> None 
  | x:: xs -> match winner_of_triple x with 
                    | Some p -> Some p
                    | None -> winner_of_list xs
...
let other_player player = function O -> X| X -> O

let place_token (player : player) (grid : grid) (row_i, col_i) : state =
...
let show_state = function
  ...
  | GameOver {result; final_grid} ->
      ...
      match result with
      |Tie -> "NIhče ni zmagal"
      |Winner p -> "Zmagal je igarlec" show_player p^"\n"
      in
...
let rec choose kind =
  ...
  | None -> 
    print_string ("Neveljavna izbira\n");
    choose kind   (* Izbrana vrednost ni veljavna. *)
...
(*izpisi stanje*)
  ...
  | OnTurn {player; grid} ->
    ...
    | Some _ ->
    (*plje je zasedeno*)
    print_string "polje je že zasedeno!\n"
    loop state
  | None ->
    (*polje je prazno*)
